...
const routes = async (fastify: FastifyInstance, options: RegisterOptions) => {
	...
	fastify.get(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			...
			redis
				...
				: reply
						.status(200)
						.send(await anilist.fetchTrendingAnime(page, perPage));
		},
	);
	...
	fastify.get(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			...
			redis
				...
				: reply
						.status(200)
						.send(await anilist.fetchPopularAnime(page, perPage));
		},
	);
	...
	fastify.get(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			const page = (request.query as { page: number }).page;
			const perPage = (request.query as { perPage: number }).perPage;
			const weekStart = (request.query as { weekStart: number | string })
				.weekStart;
			const weekEnd = (request.query as { weekEnd: number | string }).weekEnd;
			const notYetAired = (request.query as { notYetAired: boolean })
				.notYetAired;
	...
	fastify.get(
		"/recent-episodes",
		async (request: FastifyRequest, reply: FastifyReply) => {
			const provider = (request.query as { provider: "gogoanime" | "zoro" })
				.provider;
			const page = (request.query as { page: number }).page;
			const perPage = (request.query as { perPage: number }).perPage;
	...
	fastify.get(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			...
			const id = (request.params as { id: string }).id;
			const provider = (request.query as { provider?: string }).provider;
			let fetchFiller = (request.query as { fetchFiller?: string | boolean })
				.fetchFiller;
