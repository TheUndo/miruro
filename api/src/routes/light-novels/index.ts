...
const routes = async (fastify: FastifyInstance, options: RegisterOptions) => {
	...
	fastify.get(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			...
			queries.page = (
				request.query as { lightNovelProvider: string; page: number }
			).page;
			...
			const provider = PROVIDERS_LIST.LIGHT_NOVELS.find(
				(provider: any) =>
					provider.toString.name === queries.lightNovelProvider,
			);
			...
			try {
				if (provider) {
					...
					reply.status(404).send({
						message: "Page not found, please check the providers list.",
					});
				}
			} catch (err) {
