	...
	fastify.get(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			...
			queries.mangaProvider = decodeURIComponent(
				(request.params as { mangaProvider: string; page: number })
					.mangaProvider,
			);

			queries.page = (
				request.query as { mangaProvider: string; page: number }
			).page;
			...
			try {
				if (provider) {
					...
					reply.status(404).send({
						message: "Page not found, please check the provider list.",
					});
				}
			} catch (err) {
